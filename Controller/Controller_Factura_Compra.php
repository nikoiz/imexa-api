...
class Controller_Factura_Compra
{
    ...
    public function Read_single_Factura_Compra_para_detalles()
    {
        $query = "SELECT * FROM detalle_compra INNER join producto on detalle_compra.producto_id_producto=producto.id_producto where detalle_compra.id_compra=?";
        $stmt = $this->conn->prepare($query);
        //Bind id
        $stmt->bindParam(1, $this->id_compra);
